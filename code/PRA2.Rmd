...
fifaNet$Club_Position = relevel(factor(fifaNet$Club_Position), ref=max(fifaNet$Club_Position))
# Modelo lineal Multiple
multi_model_1 = lm(Rating~Age+Weight+Club_Position+Vision+Ball_Control+Marking+Interceptions+Freekick_Accuracy+Short_Pass+Speed+Finishing,data=fifaNet)
...
```{r}
intrain = createDataPartition(y=fifaNet$internacional, p=0.8, list=F)
train = fifaNet[intrain, ]
test = fifaNet[-intrain, ]
```

```{r}
model_2 = glm(formula=internacional~Age+Rating+portero+Work_Rate,data=train, family = binomial(link=logit))
```

```{r}
probabilidades = predict(model_2, test, type = "response")
predict_c = factor(ifelse(probabilidades >= 0.5, 1, 0))
confusionMatrix(data=predict_c, reference=factor(test$internacional), positive = "1")
```
